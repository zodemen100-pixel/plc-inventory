...
async function loadMaterials() {
  ...
  if (tbody) tbody.innerHTML = `<tr><td colspan="9" class="text-center">
  ...
  tbody.innerHTML = list.map(m => {
    ...
    return `<tr>
      ...
      <td class="text-center">
        ...
        <small class="text-muted"> ${esc(m.unit||'EA')}</small>
      </td>
      <td>${statusBadgeHTML(s)}</td>
      <td>
        <div class="btn-group">
          <button class="btn btn-sm btn-outline" title="바코드" onclick="showBarcode('${m.id}')">
            <i class="fas fa-barcode"></i>
          </button>
          <button class="btn btn-sm btn-primary" title="수정" onclick="openModal('${m.id}')">
            <i class="fas fa-edit"></i>
          </button>
          <button class="btn btn-sm btn-danger" title="삭제" onclick="deleteMaterial('${m.id}')">
  ...
  try {
    ...
    tree.forEach(l1 => {
      ...
      if (!(l1.children?.length)) {
        const o = document.createElement('option');
        o.value = l1.name; o.textContent = l1.name;
        sel.appendChild(o);
      }
    });
  } catch (_) {}
...
function genBarcode() {
  ...
  const bc      = [model||'MAT', series, version, dateStr, rand]
                    .filter(Boolean).join('-');
...
function showBarcode(id) {
  const m = allMaterials.find(x => x.id === id);
  if (!m || !m.barcode) {
    showToast('등록된 바코드가 없습니다.', 'warning'); return;
  }
